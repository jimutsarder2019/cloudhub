			...
			if(response.data && response.data.length > 0){
				...
				$.each( response.data, function( key, value ) {
					if(value['status']){
						let td = '';
						if(type === 'confirm'){
							td = '<td class="digits"><a href="'+base_url+'?r=voter/confirm-done&vid='+value['id']+'">Confirm</a></td>';
						}
						tr += '<tr>'+
...
function pdfPrintVote(pdfData, type) {
	...
	$.each(pdfData, function( key, value ) {
		let _class = '';
		if(value['visit_done'] && value['call_done']) { 
			 _class = 'call_visit_done';
		}
		else if(value['visit_done']) { 
			 _class = 'visit_done';
		}
		else if(value['call_done']) { 
			 _class = 'call_done';
